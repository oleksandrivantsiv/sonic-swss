...
class Resource:
    ...
    def check_treshold_exceeded(self) -> None:
        ...
        for counter, value in self.counters.items():
            if not value.get('skip_counter'):
                entry_used_counter = getCrmCounterValue(self.dvs, 'STATS', f'crm_stats_dash_{counter}_used')
                assert entry_used_counter == self.high_th, f"crm_stats_dash_{counter}_used is not equal to expected {self.high_th} value"
    ...
    def check_treshold_cleared(self) -> None:
        ...
        for counter, value in self.counters.items():
            if not value.get('skip_counter'):
                entry_used_counter = getCrmCounterValue(self.dvs, 'STATS', f'crm_stats_dash_{counter}_used')
                assert entry_used_counter == self.low_th, f"crm_stats_dash_{counter}_used is not equal to expected {self.low_th} value"
...
class AclRule(Resource):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.counters = {
            f'{self.addr_family}_acl_rule': {'th_name': 'ACL_RULE', 'skip_counter': True},
        }
